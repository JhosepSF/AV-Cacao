export function sha256(asciiOrBuffer: string | Uint8Array): string {
  const chrsz = 8;
  const hexcase = 0;
  ...
  function buf2binb(buf: Uint8Array): number[] {
    const bin: number[] = [];
    for (let i = 0; i < buf.length * 8; i += 8) {
      bin[i >> 5] |= (buf[i / 8] & 0xff) << (24 - (i % 32));
    }
    return bin;
  }
  ...
  const bin = typeof asciiOrBuffer === 'string' ? str2binb(asciiOrBuffer) : buf2binb(asciiOrBuffer);
  const len = typeof asciiOrBuffer === 'string' ? asciiOrBuffer.length * chrsz : asciiOrBuffer.length * 8;
...
export function encodeBase64(bytes: Uint8Array): string {
  ...
  if (extraBytes === 1) {
    const chunk = bytes[l];
    result += chars[(chunk >> 2) & 63];
    result += chars[(chunk << 4) & 63];
    result += '==';
  } else if (extraBytes === 2) {
    const chunk = (bytes[l] << 8) | bytes[l + 1];
    result += chars[(chunk >> 10) & 63];
    result += chars[(chunk >> 4) & 63];
    result += chars[(chunk << 2) & 63];
    result += '=';
  }
